...
- [x] **E5-16** `P1` `1d` Export the filtered view to CSV/XLSX — CSV from the rendered rows, one line per split, in the same grid. **Cut: XLSX**, same reasoning as E14-11. 0.12.0 added a PDF statement beside the CSV (`lib/statement.ts`): oldest first, money in/out columns, and opening/running/closing balances when one account is pinned and the page holds its whole period.
...
- [x] **E14-11** `P1` `2d` Export any report to CSV, XLSX, and a print-quality PDF — CSV (RFC 4180 quoted, UTF-8 BOM so Excel reads it correctly) and, since 0.12.0, a drawn PDF (jsPDF, lazy-loaded, embedded Inter font) with the report's headline figures, native charts and formatted tables — replacing print-to-PDF and the reports' Print button. **Cut: a native XLSX writer.** It needs a zip encoder and a new dependency for a format Excel already opens from the CSV; deferred rather than half-built.
